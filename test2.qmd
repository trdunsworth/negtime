...
```{r outlier_identification}
#| echo: false
#| output: false

detect_outliers <- function(data, var) {
  # Remove NA values
  clean_data <- data %>% filter(!is.na(.data[[var]]))

  # Interquartile
  Q1 <- quantile(clean_data[[var]], 0.25, na.rm=TRUE)
  Q3 <- quantile(clean_data[[var]], 0.75, na.rm=TRUE)
  IQR <- Q3 - Q1
  lower_bound <- Q1 - (1.5 * IQR)
  upper_bound <- Q3 + (1.5 * IQR)

  clean_data <- clean_data %>%
    dplyr::mutate(IQR_outlier = (.data[[var]] < lower_bound | .data[[var]] > upper_bound))

  # Modified Z-score 
  median_val <- median(clean_data[[var]], na.rm=TRUE)
  mad_val <- mad(clean_data[[var]], constant = 1.4826, na.rm=TRUE) # This adjusts for normal distribution
  mod_z_scores <- abs((clean_data[[var]] - median_val) / mad_val)

  clean_data <- clean_data %>%
    dplyr::mutate(Modified_Z_outlier = mod_z_scores > 3.5)

  # Identify rows that are outliers in both methods
  common_outliers <- clean_data %>% filter(IQR_outlier & Modified_Z_outlier)

  # Create the data frame with the specified name
  outlier_df_name <- paste0(var, "_outliers")
  assign(outlier_df_name, common_outliers, envir = .GlobalEnv)

  return(list(
    outliers = clean_data, 
    common_outliers = common_outliers,
    outlier_df_name = outlier_df_name
  ))
}
```
